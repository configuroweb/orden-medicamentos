...
class Master extends DBConnection
{
	...
	function save_product()
	{
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			if (empty($id)) {
				$resp['msg'] = 'El producto ha sido agregado con éxito';
			} else {
				$resp['msg'] = " El producto ha sido actualizado con éxito.";
			}
			...
			if (!empty($_FILES['img']['tmp_name'])) {
				...
				if (!in_array($_FILES['img']['type'], $accept)) {
					...
					if (!$uploadfile) {
						$resp['msg'] +=  " La imagen no es válida";
					} else {
	...
	function save_stock()
	{
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			if (empty($id))
				$resp['msg'] = "Stock nuevo guardado con éxito.";
			else
				$resp['msg'] = " Stock actualizado correctamente.";
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
	...
	function delete_stock()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', " Stock eliminado con éxito.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function add_to_card()
	{
		...
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "El producto ya existe en su carrito.";
		} else {
		...
		if ($resp['status'] == 'success') {
			$this->settings->set_flashdata('success', 'El producto ha sido añadido al carrito.');
		}
		return json_encode($resp);
	}
	function update_cart()
	{
		...
		if ($resp['status'] == 'success') {
			$this->settings->set_flashdata('success', 'El artículo del carrito ha sido actualizado.');
		}
		return json_encode($resp);
	}
	function delete_cart()
	{
		...
		if ($resp['status'] == 'success') {
			$this->settings->set_flashdata('success', 'El artículo del carrito ha sido eliminado.');
		}
		return json_encode($resp);
	}
	...
	function update_order_status()
	{
		...
		if ($resp['status'] == 'success')
			$this->settings->set_flashdata('success', "El estado del pedido se ha actualizado correctamente.");
	...
	function save_inquiry()
	{
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			if (empty($id))
				$this->settings->set_flashdata('success', " Su consulta ha sido enviada con éxito. ¡Gracias!");
			else
				$this->settings->set_flashdata('success', " Consulta actualizada con éxito");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
	...
	function delete_inquiry()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', " La consulta ha sido eliminada con éxito.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
