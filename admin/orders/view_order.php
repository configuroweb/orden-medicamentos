...
<div class="content py-5 px-3 bg-gradient-dark">
    <h2><b><?= isset($code) ? $code : '' ?> Información de Orden|</b></h2>
</div>
            ...
            <div class="card-header py-1">
                <div class="card-tools">
                    <?php if (isset($status) && $status < 4) : ?>
                        <button class="btn btn-info btn-sm bg-gradient-info rounded-0" type="button" id="update_status">Actualizar Estado</button>
                    <?php endif; ?>
                    <button class="btn btn-navy btn-sm bg-gradient-navy rounded-0" type="button" id="print"><i class="fa fa-print"></i> Imprimir</button>
                    ...
                    <a class="btn btn-light btn-sm bg-gradient-light border rounded-0" href="./?page=orders"><i class="fa fa-angle-left"></i> Volver</a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="container-fluid">
                    <div class="row mb-3">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="mb-3">
                                <label for="" class="control-label">Código de referencia del pedido:</label>
                    ...
                    <?php if ($order_items->num_rows <= 0) : ?>
                        <h5 class="text-center text-muted">Los artículos del pedido están vacíos.</h5>
                    <?php endif; ?>
        ...
        <div class="d-flex w-100 align-items-center">
            ...
            <div class="col-8">
                <div style="line-height:1em">
                    ...
                    <div class="text-center font-weight-bold">
                        <large>Información de Orden</large>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function() {
        ...
        $('#assign_team').click(function() {
            uni_modal("Asignar un equipo", 'orders/assign_team.php?id=<?= isset($id) ? $id : '' ?>')
        })
        $('#delete_data').click(function() {
            _conf("Deseas eliminar esta orden permanentemente?", "delete_order", ["<?= isset($id) ? $id : '' ?>"])
        })
        $('#update_status').click(function() {
            uni_modal("Actualizar Estado", "orders/update_status.php?id=<?= isset($id) ? $id : '' ?>")
        })
    })
