...
<section class="py-3">
	<div class="container">
		<div class="content py-5 px-3 bg-gradient-dark">
			<h2><b>Información de Producto</b></h2>
		</div>
				...
				<div class="card rounded-0">
					...
					<div class="card-footer py-1 text-center">
						<?php if ($_settings->userdata('id') != '' && $_settings->userdata('login_type') == 2) : ?>
							<button class="btn btn-maroon btn-sm bg-gradient-maroon text-light rounded-0" type="button" id="add_to_cart"><i class="fa fa-cart-plus"></i> Agregar al Carrito</button>
						<?php else : ?>
							<a class="btn btn-maroon btn-sm bg-gradient-maroon text-light rounded-0" href="./login.php"><i class="fa fa-cart-plus"></i> Agregar al Carrito</a>
						<?php endif; ?>
						<a class="btn btn-light btn-sm bg-gradient-light border rounded-0" href="./?p=products"><i class="fa fa-angle-left"></i> Volver</a>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(function() {
		$('#add_to_cart').click(function() {
			_conf("¿Deseas añadir este producto al carrito?", "add_cart", [])
		})
	})
